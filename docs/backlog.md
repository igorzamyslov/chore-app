...
Slices 1–3 are already planned in detail
(`docs/plans/2026-08-08-household-lifecycle-slices-1-3.md`, 1892 lines).
Nothing is implemented yet.*

| ID | Title | State | Effort |
| --- | --- | --- | --- |
| **C-1** | **Slices 1–3: server RPCs + pgTAP, the two claim-state gaps, shared confirm sheet, revocation detection** | Planned, ready to execute. Slice 1 is a HARD GATE: prove `delete from auth.users` works inside a `SECURITY DEFINER` RPC before any client work; if refused, fall back to the `sync-backend.md` §2 edge function and revisit the handoff story | L |
...
| **C-3** | Orphan household cleanup (F13) | Covered by the slice-1 migration's cascade (§2.4); listed here so it isn't tracked twice | — |
