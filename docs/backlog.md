...
**Not planned, and deliberately so:** F-2, F-3, G-2, G-5 (icon half only),
G-6, G-7, G-8 need a design conversation before a plan would be worth
writing; H-1 is a go/no-go, not code; H-2 is closed by D-B2. **G-4 has left
this list** — it has a design and a plan and shipped in wave 6. **G-5's
colour half (G-5b) has too**; only its icon half is still parked here.
...
| **G-5** | More category icons and colours (F17) | **HALF SHIPPED — read this before treating the row as done.** The **colour** half (**G-5b**) shipped 2026-08-29 (`docs/plans/2026-08-18-palette-and-ring-avatars.md`, wave 6): `CategoryRepository.palette` is twelve, both pickers draw them as theme-rendered rings six across, and `theme.dart` has hand-picked light *and* dark rows for all twelve. Additive — indices 0-7 still address exactly the colours they always did, no stored value was rewritten, schema stayed at v12. The **icon** half is **OPEN**: the nine new Material Symbols the design canvas draws in frame 1d are not implemented and the icon set is still 15 entries. Was: User request, round 1 | S |
