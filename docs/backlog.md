...
| **C-2** | **Slices 4–6: remove a claimed member (F10), leave household (F9) + last-member cascade warning, delete account (F11)** | **STILL OPEN. Planned and partly built, NOT merged** — `docs/plans/2026-08-08-household-lifecycle-slices-4-6.md` exists (the old "needs a plan" note here was stale). Attempted in wave 4 on branch `wave4/household-lifecycle-slices-4-6` / PR **#14**, which is left open with a **red `checks`** and is deliberately unmerged: nothing user-facing from slices 5–6 was reached, and a half-built delete-account flow is worse than none, since that action is irreversible server-side erasure. **What is on that branch and worth keeping:** the `exit_confirm_sheet` overflow fix, which found a SECOND defect beyond the one the handover flagged — the vertical `SingleChildScrollView` *and* a horizontal one, because Cancel plus an action label as long as "Konto löschen" does not fit 288 logical pixels at 2× text scale and a plain `Row` reports RenderFlex overflow rather than wrapping (now an `OverflowBar`, matching `AlertDialog.actions`); the three exit RPCs on `HouseholdGateway` (`removeMember` has a caller, `leaveHousehold` and `deleteAccount` do not yet); `MemberService.deleteMember` routing by claim state with `ClaimedMemberRemovalFailure`; and the `syncRefreshErrorRevoked` string plus its use on both list screens. **Two traps recorded for whoever resumes it. (1)** `member_edit_sheet.dart`'s `_delete` comment still says a throw there "would be a genuine bug … left to crash", which was true while `_canDelete` hid the action for claimed members but becomes WRONG the moment F10's UI unhides it — `ClaimedMemberRemovalFailure`'s own doc requires that failure be shown inline, since it needs the network and changed nothing. **(2)** The two red tests are probably right and the implementation incomplete: `refreshNow()` runs `_pushAll()` BEFORE `_pullSinceInner()` and `_pushAll()` throws on failure, so **any push failure preempts the revocation probe** and `membershipRevoked` is never set. That is precisely a revoked device's real behaviour — RLS starts rejecting its writes — so the "will sync later" copy would still show in production. Indicated by the code path; not confirmed by execution. Also: **the next migration is v13**, and the two-step confirm for delete-account already exists as `confirmTwoStepDestructiveAction` (`lib/features/settings/destructive_confirm.dart`) — compose a step pair, do not copy a dialog builder | L |
