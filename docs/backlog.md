...
| ID | Title | Notes | Effort |
| --- | --- | --- | --- |
| **A-6** | `e2e.yml`'s `ios` job is intermittently red on `main`, and nothing gates it | Observed at `625ea72` (run `31800958022`): 2 of 14 flows failed — `language_override` and `done_today_reopen` — both on `Assertion is false: id: welcome.create is visible`, i.e. the app was not in fresh-install state at flow start. `android` passed the same run. **Not a code regression:** `625ea72` added only `db.yml` plus `supabase/tests/**` and cannot affect iOS app behaviour, and the previous main commit passed. All 14 flows do call `clearState` and the other 12 passed, so this is state-isolation flake on the macOS runner, not a missing-`clearState` bug. **Why it matters more than a flaky test normally would:** `ios` is gated on `refs/heads/main` and NEVER runs on a pull request, so it is the one job whose failures are always discovered *after* merge, by which time attribution is guesswork. Wave 4's D-2/D-3 swipe gesture reached `main` unverified on iOS for exactly this reason. Either make the flows robust to a slow simulator, or run `ios` on PRs that touch gesture/flow code | S |
