...
Also closed: **A-7 (worktree formatter trap — fixed with a check; a
...
| --- | --- | --- | --- |
| **A-2b** | ~~Settings "Last synced" text goes stale while open~~ **CLOSED 2026-08-28** | **DONE 2026-08-28** (`docs/plans/2026-08-28-a2b-last-synced-ticker.md`): the line is now a widget of its own, `lib/features/settings/last_synced_line.dart`, which owns a one-shot `Timer` re-armed at the next moment its own text actually changes and cancelled in `dispose()`; `account_section.dart` keeps a one-line hook and the formatter moved across with it. **The tick is deliberately not periodic.** The formatter has four bands, so the gap to the next visible change is not constant: whole minutes under an hour, whole hours under a day, and past a day a fixed calendar date that nothing can alter — where **no timer is armed at all**, as when there is no cursor yet. Worst case is 60 wakes in the first hour and 23 more that day; a constant one-minute ticker would have woken 1 380 times a day to render an unchanged string. Re-arming keys on the absolute deadline, so an unrelated rebuild re-derives the same deadline and cannot postpone a pending tick, and a deadline already behind us (clock skew can put `lastPulledAt` in the future) falls back to a one-minute recheck rather than a non-positive delay, which would fire in the same frame and spin. **This row's stated blocker was wrong, and that is the finding worth carrying:** `flutter_test`'s pending-timer check runs *after* the binding unmounts the tree (`TestWidgetsFlutterBinding._runTest` calls `runApp(Container(...))` and pumps before `_verifyInvariants()` — hence the assertion's own wording, "even *after* the widget tree was disposed"), so a timer released on disposal was never the hazard, and `todayProvider`'s doc comment in `lib/app/providers.dart` states the opposite mechanism. What the check catches is a timer nothing owns. The `syncHealthStatusProvider` escape hatch ("armed only on the linked branch, so it never exists in a test") could not have been reused here anyway: `test/features/settings/account_last_synced_test.dart` renders this exact line with a live cursor, so the ticker had to be *disposable*, not *absent* — and that test is untouched, including its `findsNothing` case, which now covers the new widget's no-cursor branch. Proven by `test/features/settings/account_last_synced_ticker_test.dart`, which advances a mutable test clock and pumps fake time with **no** database write, provider invalidation or fresh `pumpWidget` between the two assertions, and additionally asserts the `State` object is the same instance afterwards — so the string cannot have changed via a rebuild from elsewhere. Both cases fail against the old code. One accepted consequence: `AppShell`'s `_KeepAlivePage` keeps a visited tab alive, so the ticker keeps running (one string format a minute) while the user is on another tab, until Settings leaves the tree. Was: Needs a periodic ticker; deliberately excluded from A-2 because a provider-armed timer trips `flutter_test`'s pending-timer check | S |
| **A-3b** | ~~iOS: the database is in `Documents/`, which iCloud backs up by default~~ **CLOSED 2026-08-28** | Fixed in `ios/Runner/AppDelegate.swift`; see the full row below for what the fix is, which two claims in this row's original wording were wrong, and what stays unverifiable. Was: The iOS counterpart to A-3's `allowBackup="false"`. Needs per-file `NSURLIsExcludedFromBackupKey` at runtime, not a manifest flag — deliberately out of A-3's scope | S |

**New rows found while executing wave 4 (2026-08-17)**

| ID | Title | Notes | Effort |
| --- | --- | --- | --- |
| **A-6** | `e2e.yml`'s `ios` job has a blank-frame flake (~1 run in 32) — **trigger settled 2026-08-28; wave 6 root-cause attempt 2026-08-29: NOT reproduced at the suite's own threshold, and the recorded cause RETRACTED as never-established** | **READ THIS BEFORE THEORISING: this row has now carried a WRONG cause once and an UNESTABLISHED one once.** (1) The original "the app was not in fresh-install state", a `clearState`/state-isolation bug — disproved 2026-08-28. (2) Its replacement, "the Flutter view permanently fails to present on a fresh install" — **retracted 2026-08-29**. That was one *reading* of run `31800958022`'s artifact, not a finding. The artifact's three facts (an app window titled "Famdo" with nothing inside it; a white screenshot; a responsive main thread snapshotting in 0.01s with zero Flutter nodes) are produced **identically** by three unrelated failures: **(a)** iOS's white `LaunchScreen.storyboard`, i.e. the engine never presented; **(b)** `ChoreApp`'s `_LoadingScaffold`, i.e. the engine DID present and `householdGateProvider` never resolved — that scaffold emitted *no accessibility node at all* (`Scaffold` contributes none; `CircularProgressIndicator`'s `Semantics` has null label and value, so it is dropped), so it photographed and dumped exactly like (a); and **(c)** an XCTest driver gone blind to an app Maestro had just uninstalled and reinstalled, which also explains the responsive main thread and the instant empty snapshot. **Only (b) would be our bug.** **WHAT WAVE 6 MEASURED.** Built a dispatch-only amplifier (`e2e.yml` input `ios_blank_frame_cycles`, probe in `e2e/amplify/`) repeating the minimal cycle — Maestro `clearState`, which on iOS is uninstall + reinstall + a genuinely cold start, then the suite's own `welcome.create` wait — with a **fresh simulator boot per 25-cycle batch**, so each batch replicates one real `ios` run. Result at the suite's own 60s threshold: **0 reproductions in 1400 cold launches across 56 fresh boots (14 runs)**. Against the historical ~0.45%/launch that is p≈0.002, and the 95% upper bound on the per-launch rate is **0.21%** — less than half the historical figure. That is genuine evidence the flake does not occur at its old rate here. **This bounds the rate on TODAY'S CI ONLY.** Every boot drew an identical fingerprint: runner image `macos26 / 20260728.0273.1`, Xcode 26.6, iPhone 17 Pro, iOS 26.2 simulator, 3-core Apple M1 (Virtual), 7 GB. It says **nothing** about the image the 2026-08 reds were drawn on. "Does not reproduce on today's CI" is NOT "fixed" — nothing was fixed; the environment moved. **TWO TRAPS, recorded so they cost nobody else.** (i) **A 20s probe is not a cheaper 60s probe.** Measured from `launchApp` it swallows process startup: one failure's launch took **12.4s** to reach the Dart VM against a **0.8s** median, so it failed on arithmetic alone; two others had rendered by ~launch+40s and **would have passed the suite's 60s gate**. All three were initially miscounted as reproductions and are not this flake. Measure at 60s. (ii) **`XCTAS Error: ... Error getting main window Unknown kAXError value -25218` is a RED HERRING.** It looks like the smoking gun; it appears in **56 of one run's 100 launches, including passing ones**, and is absent from one of the two failures it would have explained. Already ruled out. **SHIPPED, and both should stay.** `semantic('app.loading')` on the startup scaffold (spec `docs/specs/ui-foundation-chores.md`, normative id list; a *diagnostic* id — nothing taps it) — this is what lets hypothesis (b) be ruled out **by evidence** rather than argument, since a hung startup now names itself in a hierarchy dump. And the amplifier itself, dispatch-only and inert on push/PR, whose every run first **proves its own capture path fires** by forcing a failure on a healthy, fully rendered app and refusing to measure unless that produced a hierarchy, an out-of-driver screenshot, Dart VM stacks and a native `sample` — so a zero is a real zero, not a broken instrument reporting silence. The healthy baseline it records (31 nodes, ~190 KB frame) is the calibration for reading a real reproduction. **NEXT STEP if it recurs:** dispatch the amplifier and read the failure-instant artifacts — a rendered welcome screen next to an empty hierarchy proves (c) and the fix is not ours; `app.loading` in the hierarchy proves (b) and it is. **Still open; `ios` must not become a required check.** A retry or a widened timeout remains off the table: it would hide the signal, and an unverified Maestro change is exactly what the 2.7.0 pin exists to prevent | S |
| **A-7** | ~~`dart format` silently uses the wrong config in a git worktree~~ **CLOSED 2026-08-28** | A fresh worktree has no `.dart_tool/`, so `dart format` could not resolve `include: package:very_good_analysis/analysis_options.yaml`, printed a "Package resolution error" to **stderr**, exited 0, and fell back to the default `trailing_commas: automate` instead of the project's `preserve` (which is set only inside very_good_analysis's own options file, i.e. only reachable through the include that just failed). Bare `dart format .` then reflowed ~93 of 255 files. **`preserve` is *stable* on already-reflowed code, so `--set-exit-if-changed` PASSES on the churn and `ci.yml` is structurally incapable of detecting it** — measured again while fixing this: a `providers.dart` reflowed under the broken config passes `dart format --set-exit-if-changed` under a *correctly resolving* config. Cost real time in wave 4 (85 out-of-scope files in one branch, two concurrently edited by other agents). **The fix this row originally suggested — "a committed formatter config that does not depend on package resolution" — is IMPOSSIBLE; do not retry it.** An unresolvable `package:` include makes `dart format` discard the *entire* options file, so an explicit `formatter: trailing_commas: preserve` block sitting right beside the failing include is dropped with it; four YAML arrangements were measured and all four reflowed (table in the plan). Fixed instead with a check: `tool/check_formatter_config.sh` formats a fixture that comes out differently under `preserve` vs `automate` — asserting the setting that matters rather than the "does `.dart_tool` exist" proxy that would pass while the bug is live — plus a self-test that fails if the fixture ever stops discriminating. The fixture is generated at runtime into gitignored `.dart_tool/`, which `dart format`/`analyze`/`test` all skip as a dot-directory, so no future tool run can reformat it and disarm the probe. Wired as a `&&` guard *in front of* lefthook's pre-commit `format` job (before, never after — after is undetectable), into `ci.yml`, and documented in README. See `docs/plans/2026-08-28-formatter-trap.md` | XS–S |
...
**Not planned, and deliberately so:** F-2, F-3, G-6, G-7, G-8 need a
design conversation before a plan would be worth writing; H-1 is a go/no-go,
not code; H-2 is closed by D-B2. **G-4 and G-5 have both left this list** —
each had a design and a plan, and both shipped in wave 6 (G-5 in two halves,
G-5a icons and G-5b colours), and **G-2 has left it too** — it shipped in
wave 6 as the one-sentence repeat form.
...
| **A-3b** | ~~**iOS: no backup exclusion on the local DB**~~ **CLOSED 2026-08-28** | **DONE 2026-08-28** (`docs/plans/2026-08-28-a3b-ios-backup-exclusion.md`): `AppDelegate.application(_:didFinishLaunchingWithOptions:)` now sets `isExcludedFromBackup` (i.e. `NSURLIsExcludedFromBackupKey`) on the app's `Documents` directory **and** on the database's four possible file names, best-effort, on every launch, before `super` starts the Dart entrypoint. **Two claims in this row's own wording were wrong and are corrected here.** (1) The Files column named `ios/Runner/Info.plist`: there is no Info.plist key that excludes a file from backup — it is a per-URL runtime resource attribute (`URL.setResourceValues`), which the Notes column already said, so the two contradicted each other. Nothing in `Info.plist` changed. (2) There are no `-wal`/`-shm` sidecars. `drift_flutter` calls `NativeDatabase.createBackgroundConnection(file, setup: native?.setup)` with `native == null`, and drift's own `native.dart` documents that it leaves sqlite3 in the default journaling mode and that enabling WAL is the caller's job; nothing in `lib/` issues `journal_mode = WAL` (`beforeOpen` sets only `PRAGMA foreign_keys = ON`). So the reachable sidecar is the rollback journal `chore_app.sqlite-journal`, and **the torn-restore hazard as this row states it is not reachable** — in rollback-journal mode every committed byte is always in the main file. All four names are excluded anyway: a missing file is skipped, so it costs nothing and stays correct if WAL is ever switched on. **The clobber hazard IS real and this closes it**, even though the Supabase refresh token lives in `SharedPreferences`/`NSUserDefaults` rather than the database and therefore still restores — the household link, pull cursor and dirty flags are all in the database, so a device that restores the session without it lands on the welcome screen with nothing to push. **Deliberately not in Dart:** `openConnection()` is called from the F-1 background isolate as well as the main one, and that isolate's `path_provider` resolution inside a background `FlutterEngine` is the one thing in this codebase ever proven on a physical device; a Dart fix would need a new `MethodChannel` one refactor away from it, with registration Swift no more verifiable than this. Foundation-only Swift also means **Android is untouched** and there is no `Platform.isIOS` guard to get wrong. Relocating the database out of `Documents/` was rejected: iOS has no durable backup-exempt directory (`Library/Caches` is purgeable), so relocation does not even solve it, and v0.7.1 is installed on a real device so moving the file is a migration that can lose a household. **UNVERIFIED, and no CI job can change that** (A-6: `ci.yml` is format/analyze/test and the only `flutter build ios` is `e2e.yml`'s `ios` job, gated on `refs/heads/main`): that the attribute is actually set on a device (`xattr -l` on the installed container's `Documents/chore_app.sqlite` showing `com.apple.metadata:com_apple_backup_excludeItem`), and that an iCloud restore then omits the database. The function type-checks against the iPhoneOS SDK in isolation and the file parses under `swiftc -parse`; the only standing automated guard is `test/data/db/ios_backup_exclusion_test.dart`, a **name-mirror check only** — it pins the Swift's hardcoded file names to Dart's `databaseName` so a rename cannot silently un-exclude the database, and proves nothing about behaviour. Was: `chore_app.sqlite` (+ `-wal`/`-shm`) lives under `getApplicationDocumentsDirectory()` (`drift_flutter`'s native default, no iOS/Android branch), which iCloud/iTunes backs up by default — same torn-restore and cross-device session-clobber shape as A-3, but the fix is per-file (`NSURLIsExcludedFromBackupKey` at runtime), not a manifest flag, so it needs its own scoping pass. Deliberately excluded from A-3/D-B1, which is Android-only | `ios/Runner/AppDelegate.swift`, `lib/data/db/app_database.dart` (`databaseName` only) | S |
...
*Update 2026-08-29: **G-4 and G-5b now have both** — a design canvas and `docs/plans/2026-08-18-palette-and-ring-avatars.md` — and shipped in wave 6. G-5's icon half still has neither.*

> **Trap, recorded so it is not re-derived.** A design canvas shows
> **rendered** colours; `CategoryRepository.seedColors` holds **stored**
> ones. They are different numbers for the same colour, on purpose. Diffing
> a canvas against `seedColors` directly always looks like a total mismatch
> and always suggests a data migration that `docs/specs/theme-v2.md` §1.3
> forbids — which is exactly what happened when G-5b was first commissioned
> as a palette *replacement* with a v13 migration. Map through
> `categoryTone` first: against the `light` column of `theme.dart`'s
> `_categoryTones`, eight of that canvas's twelve hexes matched
> byte-for-byte, in order, and the work was an *extension*.
...
| **G-2** | Repeat-form structural redesign (F14) | **CLOSED 2026-08-29** (`docs/plans/2026-08-18-repeat-form-sentence.md`, wave 6): the repeat block is now one readable sentence with inline controls instead of a column of independent fields, plus a live preview of the next three dates. `Recurrence.monthlyDayOfMonth` was added so "the 20th of each month" can be expressed at all — it lives in the recurrence JSON, **not** a drift column, so the schema stayed at **v12**. `-1` encodes "last day", reusing `monthlyOrdinal`'s existing convention rather than a `32` that a naive 1..31 check would wave through. **Cross-version safety is the load-bearing part**: the form keeps `startDate.day` aligned to the chosen day, so a household member on a client predating the field ignores the unknown JSON key and evaluates `min(startDate.day, daysInMonth)` — byte-identical to this client's `min(monthlyDayOfMonth, daysInMonth)`. Divergence is **zero**, not merely bounded, for every day in 1..31. Only the "last day" sentinel keeps a residual: at most 3 days, always with the older client **early**, never late, and gone the moment that device updates. Was: G3 stage 2 — the wording was fixed, the structure was not | M |
...
| **G-4** | Ring avatars — colour-as-border (F15) | **SHIPPED 2026-08-29** (`docs/plans/2026-08-18-palette-and-ring-avatars.md`, wave 6). The member avatar is now two-letter initials on the neutral surface inside a ring in the member's `categoryTone`, at every size from the 24px chore tile to the 66px edit-sheet preview, scaling with text scale to a 1.6x cap. Member colours are unique per household: a colour another active member holds is inert and badged with their initials, a rule that relaxes rather than blocks once the roster outgrows the palette. **Photo avatars are NOT part of this and were never scoped** — that would be a synced blob and its own ticket. No column was added to `members`; initials derive from `name`. Was: Custom avatars — photo or colour-as-border | M |
| **G-5** | More category icons and colours (F17) | **CLOSED 2026-08-29** — both halves shipped in wave 6. The **icon** half (**G-5a**, `docs/plans/2026-08-18-category-icons.md`): the picker grew from 15 to 24 identifiers, and `test/app/theme_test.dart` now pins the `categoryIconIdentifiers`/`categoryIcon` sync, so an identifier offered without a switch case can no longer fall through to `Icons.label_outlined` and render as a plausible-looking duplicate tile. `potted_plant` maps to `Icons.local_florist`: the design canvas names Material **Symbols**, Flutter ships Material **Icons**, and that identifier does not exist in the bundled font. The **colour** half (**G-5b**, `docs/plans/2026-08-18-palette-and-ring-avatars.md`): `CategoryRepository.palette` is twelve, both pickers draw them as theme-rendered rings six across, and `theme.dart` carries hand-picked light *and* dark rows for all twelve — including one for `#9A3D80` that looks redundant but is not, since its lightness 0.4216 only just clears the 0.42 unknown-colour ceiling and without the row it silently renders a different hue (proven by inversion). **This row previously predicted 'a palette replacement with a data migration'. That was a FALSE PREMISE and was refused**: the change is additive — indices 0-7 still address exactly the colours they always did, no stored value was rewritten (theme-v2 §1.3 forbids it, because `members`/`categories` are synced tables, so a rewrite would replicate to every device), and the schema stayed at **v12**. Was: User request, round 1. Icon set unchanged at 15 entries | S |
...
| **G-12** | ~~`cancelDigest()` is unserialized against `applyDigestPlans`~~ **CLOSED 2026-08-28** | Was: carried from the v0.5.0 handover §4 as "no concurrent caller today". **F-1 changed that**: a notification action rewrites the horizon from a background isolate, so a wipe racing an action is now reachable in principle. Bounded — the worst case is the one §4 already accepts for a mid-apply process kill, a single duplicate morning notification — and documented at the call site, but no longer hypothetical. Note the fix is NOT to have the action call `cancelDigest()`: that trades one wrong notification for up to 83 days of silence. **Fixed the way `notification_action_processor.dart` specified it:** `cancelDigest()` now goes through the same serialized queue `applyDigestPlans` already used, so the two can never interleave their per-slot platform calls. `_applyTail` was renamed `_digestWriteTail` (it is the tail of all digest *writes* now, not just applies) and the three-line enqueue was extracted into one `_enqueueDigestWrite`, because it carries three properties that are easy to get silently wrong in a second copy: the tail must never be left completing with an error (or one failed write jams every write after it for the rest of the process), the error must still reach the caller that made THAT call, and **the enqueue must be synchronous** — an `async` version awaiting the predecessor before reassigning the tail would let two callers capture the same predecessor and the serialization would be vacuous. `ensureInitialized()` stays inside the serialized body for the same reason. **The ordering was verified in both directions, and both were broken before:** a cancel arriving during an apply now runs after it (nothing stays armed — before, the apply's trailing slots landed after the cancel and left a wiped app with a full armed horizon), and an apply arriving during a cancel now runs after it (a legitimate post-wipe recompute still arms — before, the cancel's trailing ids wiped the apply's work and silenced the digest). Both directions have a test that fails against the old code; two further tests pin the error properties. **Deliberately NOT extended across isolates** — the neighbouring per-instance hazard stays recorded and unfixed, as `notification_action_processor.dart` and `docs/specs/notifications.md` both require. One accepted consequence, recorded at `reset_flow.dart`'s `_cancelDigest`: the wipe's cancel may now wait behind an in-flight apply, which does not weaken the "never block the wipe" promise beyond what was already true, since that call always awaited `plugin.initialize()`. See `docs/plans/2026-08-28-g12-cancel-digest-serialization.md` | S |

| **G-13** | 12sp category labels do not clear 4.5:1 in the light theme | Only 3 of the 8 original light renders in `theme.dart`'s `_categoryTones` actually reach the 4.5:1 that `docs/specs/theme-v2.md` §1.3's own rationale claims for them; the light floor is 3.30 (`#B96A4C`). **Pre-existing** — true on `main` before the G-5b widening, which neither introduced nor worsened it (its four new colours are the best of the twelve, at 4.48–5.15). 3:1 is the right bar for a *ring* (a UI edge), which is what G-4 draws and what `test/app/palette_test.dart` asserts in both themes; 4.5:1 is the right bar for the 12sp *label*, and that is the gap. Fixing it means re-picking light renders for up to five stored colours — render-time only, since the stored values must never be rewritten (§1.3) — and re-checking perceptual separation, which the widened palette has already tightened to ΔE 7.8. Deliberately left out of the G-4/G-5b plan | S |

| **G-14** | Widget tests cannot measure real text metrics | `flutter test` draws the Ahem-style `FlutterTest` font (~1 em per glyph), so any assertion about whether text physically fits — glyph advance, wrapping, truncation — is either vacuous or false. Measured during G-4: `'WM'` at 11px/w600 is **21.87px** in tests against ~15px in the shipping Inter, which made the ring-avatar fit assertion unassertable and left glyph fit as a visual-QA gate. A `FontLoader` in `setUpAll` does NOT fix it: the faces are declared under pubspec `fonts:` rather than `assets:`, so `rootBundle.load` returns nothing usable and the loader silently no-ops. The fix is a `test/flutter_test_config.dart` that adds the four Inter TTFs to `assets:` and loads them once for the whole suite. Worth doing the next time a test genuinely needs text metrics — not before, since it would also change the layout every existing widget test sees | S |
| **G-15** | The icon picker grid does not fill the screen width on Android | Found on a real device by Igor, 2026-08-30 — the first device look at wave 6. The G-5a plan asserted "six across" already held with **zero layout code change**, and verified it against widget-test geometry; on a real Android screen the block is narrower than the sheet. Note **G-14**: widget tests draw the Ahem-style `FlutterTest` font and cannot measure real metrics, so the check that cleared this could not have caught it. Diagnose the actual constraint (fixed tile size vs the sheet's own padding) before changing anything | `lib/features/settings/category_edit_sheet.dart` (`_IconGrid`) | S |
| **G-16** | Two-letter initials touch the avatar border at small sizes | Found on a real device by Igor, 2026-08-30. Two letters was **my ruling, not the plan's** — the planner recommended one letter and I overruled it, because a unique ring colour cannot be the sole identity channel for a colour-blind user and one letter collides for Anna/Alex. The obligation I attached was a legibility check at 16px and text scale 2.0; the avatar went 20px→24px on that basis. But wave 6's own report lists that fit check among **four tests that could not fail** — text is constrained to the ring, so it reports its own constraint and passes at any geometry. So the fit was never measured, only asserted. **Measure it first** (G-14 says widget tests cannot, so this needs a device or a golden), then either give two letters real optical padding at every size in use, or drop to one letter — Igor's own suggestion — and record what the accessibility trade costs | `lib/features/members/member_avatar.dart` | S |
