...
void main() {
  ...
  test('reconnect (spec §7.6): no code, no claim/join RPC -- downloads and '
      'replaces straight from the ReconnectChoice ids', () async {
    final gateway = FakeHouseholdGateway()
      ..downloadSnapshotOverride = const HouseholdSnapshot(
        household: Household(
          id: 'joined-hh',
  ...
  group('unconfirmed snapshots never replace local data (spec §7.6)', () {
    ...
    Future<void> expectNothingDestroyed() async {
      expect((await db.select(db.households).get()).map((row) => row.id), [
        'old-hh',
      ]);
      expect((await db.select(db.members).get()).map((row) => row.id), [
        'old-m1',
      ]);
      expect((await db.select(db.categories).get()).map((row) => row.id), [
        'old-c1',
      ]);
      expect((await db.select(db.chores).get()).map((row) => row.id), [
        'old-ch1',
      ]);
      expect(
      ...
      expect((await db.select(db.shoppingItems).get()).map((row) => row.id), [
        'old-s1',
      ]);
    ...
    test('reconnect against a household the caller can no longer read aborts '
        'and destroys nothing', () async {
