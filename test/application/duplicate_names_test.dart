...
void main() {
  ...
  group('two chores sharing a title', () {
    test(
      'completing one closes only that occurrence; the sibling stays '
    ...
    test(
      'skipping one advances/closes only that occurrence; the sibling '
    ...
    test(
      "pausing one removes only that chore's pending occurrence; the "
      'sibling stays pending and unpaused',
      () async {
        final m1 = await _insertMember(db, 'm1', householdId);
        final m2 = await _insertMember(db, 'm2', householdId);
        final choreA = await serviceOn(PlainDate(2026, 1, 1)).createChore(
    ...
    test(
      "soft-deleting one removes only that chore's pending occurrence; "
      'the sibling stays active and untouched',
      () async {
        final m1 = await _insertMember(db, 'm1', householdId);
        final m2 = await _insertMember(db, 'm2', householdId);
        final choreA = await serviceOn(PlainDate(2026, 1, 1)).createChore(
    ...
    test(
      'reopening a closed occurrence restores the correct one; the '
      'sibling (closed the same day) stays closed',
      () async {
        final m1 = await _insertMember(db, 'm1', householdId);
        final m2 = await _insertMember(db, 'm2', householdId);
        final choreA = await serviceOn(PlainDate(2026, 1, 1)).createChore(
