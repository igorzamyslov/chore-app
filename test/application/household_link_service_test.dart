...
void main() {
  ...
  test('disconnect clears syncHouseholdId, syncLinkedAt, and '
      'syncLastPulledAt, leaves every other local row untouched, and never '
      'calls the gateway (the server is never touched)', () async {
  ...
  test('adopt mirrors the server claim onto the local member row (G-B, spec '
      'docs/specs/household-lifecycle.md §3.1) without dirtying it', () async {
    final household = await HouseholdRepository(db).createLocalHousehold('Me');
    final me = await (db.select(
      db.members,
    )..where((tbl) => tbl.householdId.equals(household.id))).getSingle();
  ...
  test("adopt sets settings.actingMemberId to the caller's own member id, "
      ...
      'docs/specs/members-management.md §4.2, pre-claim window)', () async {
    final household = await HouseholdRepository(db).createLocalHousehold('Me');
    final me = await (db.select(
      db.members,
    )..where((tbl) => tbl.householdId.equals(household.id))).getSingle();
