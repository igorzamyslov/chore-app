...
void main() {
  ...
  test('watchMembers orders by name and reacts to additions', () async {
    final household = await repo.ensureLocalHousehold();
    final emissions = <List<String>>[];
    final sub = repo
        .watchMembers(household.id)
        .listen((members) => emissions.add([for (final m in members) m.name]));
    ...
    expect(emissions.last, ['Alice', 'Me', 'Zoe']);
  });
