...
void main() {
  ...
  test('a fresh (never-opened) database is created at the current '
      'schemaVersion (11) directly, settings table included with every '
      'column -- membershipRevoked among them', () async {
  ...
  test('schemaVersion 3 -> 11 upgrade adds locale, both shown-once flags, the '
    ...
    await seed.customStatement('ALTER TABLE settings DROP COLUMN theme_mode');
    await seed.customStatement(
  ...
  test('schemaVersion 2 -> 11 upgrade adds every later settings column, '
      ...
      'supersedes earlier per-step tests', () async {
    final dir = await Directory.systemTemp.createTemp(
  ...
  test('schemaVersion 5 -> 11 upgrade adds syncHouseholdId, syncLinkedAt, '
      'themeMode, syncLastPulledAt, and membershipRevoked (NULL/false by '
      'default, no data rewrite), keeping the existing settings row', () async {
    final dir = await Directory.systemTemp.createTemp(
  ...
  test('schemaVersion 6 -> 11 upgrade adds themeMode, syncLastPulledAt, and '
      'membershipRevoked (NULL/false by default, no data rewrite), keeping '
      'the existing settings row', () async {
    final dir = await Directory.systemTemp.createTemp(
  ...
  test('schemaVersion 7 -> 11 upgrade adds syncDirty (default false) to every '
      'synced table and syncLastPulledAt/membershipRevoked (NULL/false) to '
      'settings, keeping existing rows', () async {
    final dir = await Directory.systemTemp.createTemp(
    ...
    final assignee = await upgraded.select(upgraded.choreAssignees).getSingle();
  ...
  test('schemaVersion 8 -> 11 upgrade adds members.deletedAt and '
      'settings.membershipRevoked (NULL/false by default, no data rewrite), '
      'keeping the existing member row', () async {
    final dir = await Directory.systemTemp.createTemp(
  ...
  test('schemaVersion 9 -> 11 upgrade adds settings.membershipRevoked '
